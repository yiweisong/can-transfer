...
def get_device_uart_transfer(provider:str, options: UartOptions):
    if provider == 'lg69t':
        return LG69TUartTransfer(options)
    elif provider == 'rtk350la':
        return RTK350LATransfer(options)
    elif provider == 'ins330':
        return INS330Transfer(options)
    else:
        return None
...
class TransferBase:
    ...
    def send_vehicle_speed(self, speed: int, gear: int):
        pass

class RTK350LATransfer(TransferBase):
    ...
    def __init__(self, options: UartOptions) -> None:
        ...
        if not self._transfer:
            raise Exception(
                'Cannot connect the RTK350LA Uart transfer, please check the configuration')
...
class INS330Transfer(TransferBase):
    _transfer: UartTransfer
    ...
    def __init__(self, options: UartOptions) -> None:
        ...
        if not self._transfer:
            raise Exception(
                'Cannot connect the INS330 Uart transfer, please check the configuration')

    def on_initalize(self):
       pass

    def send_vehicle_speed(self, speed: int, gear: int):
        # build uart command OD(0x4f 0x44), gear, speed
        message = message_helper.build_uart_command(
            bytes([0x4f, 0x44]), list(struct.pack('<b', gear)) + list(struct.pack("<f", speed))
        )
        self._transfer.send(message)
...
class MTLT335DCanTransfer(TransferBase):
    def __init__(self, config) -> None:
        super().__init__()
        can_bus_config = AppContext.config['can_bus']
        self.__transfer = CanTransfer(CanOptions(can_bus_config['channel'], can_bus_config['bitrate']))
        self.__speed_log = app_logger.create_logger('can_proxy')
    ...
    def send_vehicle_speed(self, speed: int, gear: int):
        if speed < 0:
            return
        #prepare speed message, PGN 65215
        speed_bytes = int(speed*256).to_bytes(2, byteorder='little')
        message_content = speed_bytes + bytes([0x7D, 0x7D, 0x7D, 0x7D, 0x7D, 0x7D])
        self.__speed_log.append(' '.join(['0x{0:02X}'.format(x) for x in message_content]))
        msg = can.Message(
            arbitration_id=0x18FEBF00, 
            is_extended_id=True, 
            data=message_content)
        self.__transfer.send(msg)
        
        #prepare gear message, PGN 61445
        gear_bytes = (gear+125).to_bytes(1, byteorder='little')
        message_content = bytes([0x00, 0x00, 0x00])+ gear_bytes + bytes([0x00, 0x00, 0x00, 0x00])
        self.__speed_log.append(' '.join(['0x{0:02X}'.format(x) for x in message_content]))
        msg = can.Message(
            arbitration_id=0x18F00500, 
            is_extended_id=True, 
            data=message_content)
        self.__transfer.send(msg)
...
class TransferFactory:
    def create(config):
        ...
        if protocol == 'uart':
            ...
            transfer = get_device_uart_transfer(provider, options)
            if not transfer:
                raise Exception(
                    'Cannot connect the {0} Uart transfer, please check the configuration'.format(provider))

            transfer.on_initalize()
            return transfer

        if protocol == '100base-t1' and provider in ['ins401','ins402']:
            transfer = INS401EthernetTransfer()
            transfer.on_initalize()
            return transfer
        
        if protocol == 'can' and provider in ['mtlt335d']:
            transfer = MTLT335DCanTransfer(config)
            transfer.on_initalize()
            return transfer
...
class TransferCenter:
    ...
    @property
    def can_continue_work(self):
        return len(self._transfers)>0
    ...
    def dispatch_vehicle_speed(self, speed, gear):
        for transfer in self._transfers:
            transfer.send_vehicle_speed(speed, gear)
